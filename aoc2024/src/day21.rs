...
pub fn solve_part2(input: &str) -> usize {
    ...
    for code in all_moves {
        for i in 0..n_steps {
            find_min_len_code2(&code, i, &mut memo);
        }
    }

    for code in input {
        let steps = short_len_num_code2(code, &find_all_moves);
        let len = steps
            .into_iter()
            .map(|code| {
                let mut len = 0;
                ...
                while let Some(end) = code2.pop() {
                    ...
                    len += possible_moves
                        .iter()
                        .map(|s| find_min_len_code2(s, n_steps, &mut memo))
                        .min()
                        .unwrap();
                    start = end;
                }
...
fn find_min_len_code2(
    ...
    let next_steps = short_len_code2(code);
    let min_len = next_steps
        .into_iter()
        .map(|s| {
            s.into_iter()
                .map(|s| find_min_len_code2(s, steps - 1, memo))
                .min()
                .unwrap()
        })
        .sum();
...
fn short_len_code2(code: &str) -> Vec<Vec<&str>> {
    let mut start = 'A';
